...
\item \red{Construire la table}
	...
	\end{figure}
\item \red{Prendre la diagonale} : $d(n) = interpret(n,n)$
\item \red{Modifier la diagonale} : $d'(n)= interpret(n,n) + 1$ (calculable dans $Q$ si $interpret$ calculable dans $Q$)
\item \red{Contradiction} : $d'(d)= interpret(d,d) + 1 \text{ \textblue{OR} } d'(d) = \varphi'(d) = interpret(d,d)$
\item \green{Conclusion} : l’interpréteur de $Q$ n'est pas calculable dans $Q$
\end{enumerate}
...
Un ensemble $A$ est \textblue{algorithmiquement réductible} à un ensemble $B \ (A \leq_a B)$ si en supposant $B$ récursif, $A$ est récursif.
...
Un ensemble $A$ est \textblue{fonctionnellement réductible} à un ensemble $B \ (A \leq_r B)$ ssi il existe une fonction totale calculable $f$ t.q. :
\begin{equation*}
...
Un ensemble $A$ est \textblue{polynomialement réductible} à un ensemble $B \ (A \leq_p B)$ ssi il existe une fonction totale calculable $f$ de complexité temporelle polynomiale t.q. :
\begin{equation*}
a \in A \Leftrightarrow f(a) \in B
\end{equation*}
