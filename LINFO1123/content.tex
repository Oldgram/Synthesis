...
Un \textblue{ensemble} est une collection d'objets, sans répétition, appelés les éléments de l'ensemble.\\
Exemples : ensemble vide = $\emptyset$, ensemble fini = $\{1, 2, 3\}$, ensemble infini = $\{1, 2, 3, ...\}$, complément de $A = \bar{A}$.

\subsection{Langages}

Un \textblue{langage} est un ensemble de mots constitués de symboles d'un \textblue{alphabet} donné.\\
Exemple : palindromes sur $\{a, b\}$ : $\epsilon, a, b, aa, aaa, baab, babab, ...$

Un \textblue{alphabet $\Sigma$} est un ensemble de symboles.\\
Exemples : $\Sigma = \{0, 1\}, \Sigma = \{a, b, c\}$

Une \textblue{chaîne de caractères}, ou mot, est une séquence finie de symboles juxtaposés.\\
Exemples : $010110, abccbcabcb, \epsilon$ (= chaîne de caratères vide)\\
L'ensemble de tous les mots de l'\textblue{alphabet $\Sigma$} s'écrit $\Sigma*$\\
Exemples : $\Sigma = \{\} \rightarrow \Sigma* = \{\epsilon\}, \Sigma = \{a\} \rightarrow \Sigma* = \{\epsilon, a, aa, aaa, aaaa, ...\}$

\subsection{Relations et fonctions}

Une \textblue{relation} $R$ sur $A, B$ est un sous ensemble de $AxB$. En pratique, on dit que c'est une \textblue{fonction}.

Une \textblue{fonction $f$} (de A dans B) est une relation t.q. pour $a \in A$, il existe au plus un $b \in B$ t.q. $<a, b> \in f$.

\subsubsection{Propriétés des fonctions}

Soit $f : A \rightarrow B$ :
\begin{itemize}
\item \textblue{Domaine} de $f$ : dom($f$) $= \{a \in A \ | \ f(a) \neq \bot\}$
\item \textblue{Image} de $f$ = im($f$) $= \{b \in B \ | \ \exists a \in A : b = f(a)\}$
\item $f$ est fonction \textblue{totale} ssi dom($f$) $= \ A \ (\nexists a \in A : f(a) = \bot)$ 
\item $f$ est fonction \textblue{partielle} ssi dom($f$) $\subseteq \ A$ ($f$ totale est partielle mais $f$ partielle n'est pas forcément totale)
\item $f$ est \textblue{surjective} ssi im($f$) $= B$ (tout $y$ a au moins un $x$)
\item $f$ est \textblue{injective} ssi $\forall a, a' \in A : a \neq a' \rightarrow f(a) \neq f(a')$ (tout x a un y différent)
\item $f$ est \textblue{bijective} ssi $f$ est totale, injective et surjective (tout y a un et un seul x)
\end{itemize}

\section{Ensembles énumérables}

Un ensemble est \textblue{énumérable} s'il est soit fini ou s'il a le même \textblue{cardinal}\footnote{Deux Ensembles $A$ et $B$ ont le même cardinal s'il existe une bijection entre $A$ et $B$.} que $\mathbb{N}$\footnote{L'ensemble $\mathbb{N}$ est l'ensemble des entiers positifs $\mathbb{N} = \{0, 1, 2, 3, \ldots\}$.} (si on peut le mettre en bijection avec $\mathbb{N}$). En informatique, un programme est une chaîne finie de caractères $\rightarrow$ énumérable.
...
Soit $E = \{x \in \mathbb{R} \ | \ 0 < x \leq 1\}$. \textblue{$E$ est non énumérable}\footnote{Il n'existe pas de bijection entre $E$ et $\mathbb{N}$ c.à.d. qu'il y a beaucoup plus de réels entre 0 et 1 qu'il n'y a d'entiers positifs.}

\textblue{Preuve} :
\begin{enumerate}
\item \red{Supposons $E$ énumérable}. Il existe donc une énumération des éléments de $E : x_0, ..., x_k, ...$
\begin{figure}[H]
    	\centering
...
Un \textblue{algorithme} est une procédure qui peut être appliquée à n'importe quelle donnée (appartenant à une classe de données symboliques) et qui a pour effet de produire un résultat (appartenant à une classe de données symboliques). C'est un ensemble fini d'instructions (programme) \textblue{calculant} une fonction (algorithme $\neq$ fonction)
...
\textblue{$halt$ n'est pas calculable.}
...
\item \textblue{$interpret(n, x)$ n'est pas calculable \textit{dans $Q$}}
\end{itemize}
...
\begin{enumerate}
\item \textblue{Construire la table}
	\begin{figure}[H]
    		\centering
    		\begin{tabular}{c|cccccc}
		 & 0 & $\cdots$ & $k$ & $\cdots$ \\ 
	...
	\end{figure}
\item \textblue{Prendre la diagonale} : $d(n) = interpret(n,n)$
\item \textblue{Modifier la diagonale} : $d'(n)= interpret(n,n) + 1$ (calculable dans $Q$ si $interpret$ calculable dans $Q$)
\item \textblue{Contradiction} : $d'(d)= interpret(d,d) + 1 \text{ \red{OR} } d'(d) = \varphi'(d) = interpret(d,d)$
\item \textblue{Conclusion} : $interpret(n, x)$ n'est pas calculable dans $Q$
\end{enumerate}
