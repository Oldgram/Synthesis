...
Une fonction $f : \mathbb{N} \rightarrow \mathbb{N}$\footnote{Se lit : "une fonction de $\mathbb{N}$ dans $\mathbb{N}$". L'ensemble $\mathbb{N} \rightarrow \mathbb{N}$ est non énumérable (l'ensemble des problèmes a un plus grand cardinal que l'ensemble des programmes)} est calculable ssi il existe un programme qui, recevant comme données n'importe quel nombre naturel $x$, fourni comme résultat $f(x)$ s'il est défini, sinon $\bot$ (s'il ne se termine pas ou erreur).
...
\item \textred{$interpret(n, x)$ n'est pas calculable dans $Q$}
\end{itemize}
...
\section{L'ensemble des fonctions totales n'est pas énumérable}
