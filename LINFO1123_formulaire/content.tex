...
\item Si le domaine d’une fonction est fini, alors cette fonction est calculable : \textgreen{Vrai} % Q21
\item Si SAT $\leq_a A$ et $A \in P$, alors SAT $\in P$  : \textred{Faux} % Q108
\item L’ensemble des sous-ensembles récursivement énumérables de $\mathbb{N}$ est énumérable : \textgreen{Vrai} % Q18
\item Tout ensemble de paires d’entiers est récursif : \textred{Faux} % Q17
\item L’ensemble des programmes Java calculant une fonction $f$ telle que $f(10)=10$ est un ensemble récursif : \textred{Faux} % Q25
\item Un sous-ensemble infini d’un ensemble récursivement énumérable est récursivement énumérable : \textred{Faux} % Q7
\item Soit $D$ un nouveau modèle de calculabilité. Si toute fonction calculable est calculable dans $D$ et si toute fonction calculable dans $D$ est effectivement calculable, alors $D$ est un modèle complet de la calculabilité : \textred{Faux} % Q111
\item Un algorithme donné ne calcule qu’une et une seule fonction  : \textgreen{Vrai} % Q12
\item Un sous-ensemble d’un ensemble récursivement énumérable est récursivement énumérable : \textred{Faux} % Q19
\item Un problème qui peut être résolu par un algorithme (de complexité) exponentiel est pratiquement infaisable (intrinsèquement complexe) : \textred{Faux} %Q84
\item Le complément d’un ensemble récursivement énumérable est récursivement énumérable : \textred{Faux} %Q10
\item Soient les programmes $P_{32}(n)$ dont le code est « print(1) » et $P_{57}(n)$ dont le code est « print(0) » : %Q32
	...
	\end{itemize}
\item Les propriétés SA, CD et S sont suffisantes pour que D soit un modèle complet de la calculabilité : \textgreen{Vrai} %Q113
\item Un ensemble est f-réductible (fonctionnellement réductible) à son complément : \textred{Faux} %Q88
\item Si A peut être décidé par un algorithme polynomial et si B est f-réductible à A, alors B peut être décidé par un algorithme polynomial : \textred{Faux} %Q89
\item Tout ensemble récursivement énumérable est a-réductible à HALT : \textgreen{Vrai} %Q87
\item Un ensemble énumérable est récursif : \textred{Faux} % Q2
\item Si A est un sous-ensemble (strict et non vide) récursif de programmes Java, alors toute fonction calculée par un programme de A est aussi calculée par un programme du complément de A : \textred{Faux} %Q28
\item Soit A est un ensemble (infini) récursivement énumérable. Si B $\subseteq$ A, alors B est aussi récursivement énumérable : \textred{Faux} %Q39
\item Il existe un ensemble infini de chaînes finies de caractères (A-Z) qui est non récursivement énumérable : \textgreen{Vrai} % Q36
\item Une extension d’une fonction partielle calculable est toujours calculable : \textred{Faux} % Q33
\item Le modèle de calcul BLOOP possède les propriétés SD et SA : \textgreen{Vrai} % Q110
\item L’union d’une infinité énumérable d’ensembles récursivement énumérable est récursivement énumérable : \textred{Faux} % Q9
\item Pour déterminer si un problème A est NP-complet, il suffit de déterminer que A est polynomialement réductible un problème NP-complet connu (e.g. SAT) : \textred{Faux} % Q97
\item Il existe des ensembles récursifs qui ne sont pas récursivement énumérables : \textred{Faux} % Q97
\item Un sous-ensemble infini d’un ensemble récursif est récursif : \textred{Faux} % Q3
\end{itemize}
